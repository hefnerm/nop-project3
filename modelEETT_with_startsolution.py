...
def solve_EETT(trainDic, powerDic, T_m, PL, ST, PassConOrd, timeHorizonMin, instance):
	...
	#start_solution
	with codecs.open("./solutions_greedy_heuristic_store/solution_greedy_heuristic_instance" + str(instance) + ".json.txt", "r", encoding="utf-8") as infile:
	...
	for j in legList:
		...
		for t in TLegs[j['LegID']]:
			if t==solutionHeuristic['Legs'][j['LegID']]:
				solutionHeuDic[j['LegID']] = 1
				print(solutionHeuristic['Legs'][j['LegID']])
			else:
				solutionHeuDic[j['LegID']] = 0
